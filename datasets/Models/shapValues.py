...
# # 2. Create a DataFrame to store SHAP values
# shapDF = pd.DataFrame(index=ids, columns=XtrainReduced.columns)

# for idx, row in enumerate(ids):
# # Compute SHAP values for the current row
#     shap_values = explainer.shap_values(XtrainReduced.loc[row].values.reshape(1, -1))[0]
#     shap_values_abs = np.abs(shap_values)
#     shapDF.loc[row] = shap_values_abs

# # 3. Naming folders
# test = sys.argv[1].split("_")[-4].split(".")[0]
# datasetType = os.path.basename(sys.argv[1]).split("_")[0]
# outputFileName = f"{subscore}_{test}_{datasetType}_{target}_shap_ABS_DF.csv"
# shapDF.to_csv(outputFileName, index=True)

# Deriving explanation for a given prediction
shapValues_test = explainer(XtrainReduced.iloc[[0]])
